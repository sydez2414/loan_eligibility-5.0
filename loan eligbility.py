...
with st.form("loan_form"):
    ...
    property_price = st.number_input("Harga Hartanah (RM)", min_value=10000.0, step=1000.0, format="%0.2f")
    tenure = st.slider("Tempoh Pembiayaan (Tahun)", min_value=5, max_value=35, value=30)
    income = st.number_input("Pendapatan Bersih Bulanan (RM)", min_value=0.0, step=100.0, format="%0.2f")
    commitment = st.number_input("Jumlah Komitmen Bulanan (RM)", min_value=0.0, step=50.0, format="%0.2f")

    co_name = st.text_input("👥 Nama Pembeli Bersama (Jika Ada)")
    co_income = st.number_input("Pendapatan Bersih Pembeli Bersama (RM)", min_value=0.0, step=100.0, format="%.2f")
...
if submitted:
    ...
    for bank, rate in banks.items():
        ...
        total_commitment = commitment + 1500 + installment
        dsr = total_commitment / total_income if total_income > 0 else 0
    ...
    df_result["Ansuran (RM)"] = df_result["Ansuran (RM)"].apply(lambda x: f"{float(x.replace(',', '')):,.2f}")
    ...
    # Butang Share
    st.markdown("### Kongsi Laporan")
    col3, col4 = st.columns([1, 1])
    with col3:
        st.markdown(f"[📤 Share WhatsApp](https://wa.me/?text=Laporan%20kelayakan%20telah%20dihasilkan%20untuk%20anda...)")
    with col4:
        email_url = f"mailto:{client_email}?subject=Hasil%20Semakan%20Kelayakan&body=Laporan%20kelayakan%20telah%20dilampirkan."
        st.markdown(f"[✉️ Hantar ke Emel]({email_url})", unsafe_allow_html=True), file_name="laporan_kelayakan.pdf", mime="application/pdf")

    # Butang Amortization
    if st.button("📅 Amortization"):
        st.subheader("📅 Jadual Amortization (Ringkasan)")
        r = (float(df_result.iloc[0]['Kadar (%)']) / 100) / 12
        n = tenure * 12
        loan_amount = property_price * 0.9
        monthly = loan_amount * r * (1 + r)**n / ((1 + r)**n - 1) if r > 0 else loan_amount / n
        total = monthly * n
        interest = total - loan_amount
        st.write(f"Jumlah Bayaran Semula: RM{total:,.2f}")
        st.write(f"Jumlah Faedah: RM{interest:,.2f}")
